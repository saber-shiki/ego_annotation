...
Runtime agents must not share the parent agent's active cwd or dirty working tree. Launch a runtime agent from an isolated cwd, preferably a dedicated clean git worktree, and keep generated prediction artifacts under a fresh run root. If the checked-in runtime prompt/system prompt includes Workbench, `.memory/tasks`, OPS/EPISTEMIC, parent-session language, GT/evaluation, or development checklists as runtime instructions, stop and repair the runtime entry artifacts before launch.
