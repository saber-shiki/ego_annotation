...
During agent command execution, do not use foreground `sleep` to wait for job completion. Do not run `sleep 30`, `sleep 60`, `sleep 180`, polling loops, or any equivalent idle wait as a progress substitute. Long-running work must run in tmux or a job system with durable sentinels/logs; after launch, either work on an independent blocker, perform an immediate non-blocking status/log check, or yield the job handle. The only permitted agent-side `sleep` is a sub-second TTY/keystroke settling delay required by an interactive tool, never a completion wait and never in a loop.
