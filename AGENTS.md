...
Use a single tmux session per active project task. For V18 work, the canonical agent session is `ego_annotation_v18_workbench`. Create multiple windows inside that session for separate commands or long-running jobs; do not create one tmux session per branch, run, validator, probe, or script. Before launching a command, reuse the canonical task session or create it if missing. If project-task session sprawl is found, preserve active state by moving windows into the canonical session and remove the extra project sessions. Tmux is execution containment, not evidence of progress.
