...
Use a single tmux session per project. For this project, the canonical agent session is `ego_annotation`. Create multiple windows inside that session for separate commands or long-running jobs; do not create one tmux session per task, branch, run, validator, probe, or script. Before launching a command, reuse the canonical project session or create it if missing. If project-session sprawl is found, preserve active state by moving windows into the canonical session and remove the extra project sessions. Tmux is execution containment, not evidence of progress.
