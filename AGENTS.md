...
2. Hand branch: run HaWoR, WiLoR, and RTMLib on the full video. HaWoR is the required temporal/occlusion hand baseline; WiLoR is the visible-frame MANO candidate; RTMLib is the independent 2D keypoint anchor. WiLoR, HaMeR, MANO2D, depth probes, validators, or overlays cannot satisfy the HaWoR requirement or be reported as a substitute for it. The branch must preserve and expose metric 3D MANO joints, vertices or reproducible MANO parameters, camera/world transforms, source intrinsics, and confidence/provenance before any contact/occlusion/nonpenetration reasoning. Compare hand candidates using 2D keypoint reprojection, metric-depth disagreement, temporal acceleration, hand bone-scale plausibility, and visual judgment as confidence evidence. Emit approximate hand candidates or explicit unresolved states for every frame; do not suppress the hand branch because local confidence is low.
