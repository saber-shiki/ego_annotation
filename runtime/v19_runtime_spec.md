...
P17 rows are semantic/visibility priors, not object-pose observations. Their existence or high `contact_prior_probability` cannot promote an unready P15 trajectory, recover missing metric object poses, or bypass the P16/P18 quarantine. The v2 factor also binds an independent P09 accepted-first-surface support mask/depth NPZ for translation grounding. This support is separate from `constraint_eligible_entity`, never reads raw depth under projected MANO, and does not by itself create a one-sided hand-behind-object residual.
...
Signed object-surface active-set and dense barriers require an actually watertight physical surface, `geometry_readiness.signed_geometry_ready: true`, and a valid mesh-bound local face-authority contract when the source is a shared observation-only proxy. Otherwise they remain explicitly inactive; unsigned local contact-surface and direct visible depth-order terms may remain available under their own evidence gates. `--allow-unready-signed-geometry-for-diagnostic-optimization` is a historical diagnostic override only and must quarantine the resulting state. The in-solver translation gate freezes root translation when independent P09 first-hit grounding is absent; the output gate then preserves source HaWoR wrist/root translation while keeping optimized wrist-relative articulation. Supported rows must not be reverted. P18 must separately report raw optimizer and published candidate residuals, and full-MANO promotion must fail if any output gate was applied or if topology penetration remains nearest a face without local authority.
