...
def validate_case(case_report: dict[str, Any], report_text: str) -> dict[str, Any]:
    ...
    require(overlay_object_supported + overlay_object_rejected + overlay_object_unvalidated > 0, f"{case}: overlay rendered no reconstructed geometry pose labels")
    require(world_object_supported + world_object_rejected + world_object_unvalidated > 0, f"{case}: world render drew no reconstructed mesh footprints")
