...
def project(points, K):
    return points[..., :2] / np.maximum(points[..., 2:], 1e-6) * K[:2] + K[2:]


def resized_K(K, source=(1408, 1408), target=(960, 960)):
    s = np.asarray(target) / np.asarray(source)
    return np.r_[np.asarray(K)[:2] * s, (np.asarray(K)[2:] + 0.5) * s - 0.5]
...
def mano_models(path, device="cpu"):
    ...
    models = {
        side: smplx.MANO(
            str(Path(path) / f"MANO_{side.upper()}.pkl"),
            is_rhand=(side == "right"),
            use_pca=False,
            flat_hand_mean=True,
        ).to(device)
...
def torch_project(p, K):
    return p[..., :2] / p[..., 2:].clamp_min(0.01) * K[..., :2] + K[..., 2:]
...
def closest(scene, points):
    ...
    shape = points.shape
    out = scene.compute_closest_points(
        o3d.core.Tensor(np.asarray(points, dtype=np.float32).reshape(-1, 3)), nthreads=4
    )
    q = out["points"].numpy().reshape(shape)
    n = out["primitive_normals"].numpy().reshape(shape)
    return q, n
