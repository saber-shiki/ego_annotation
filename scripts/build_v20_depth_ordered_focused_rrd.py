...
def depth_order_faces(
    ...
    *,
    ...
    # Direct ray-wise pruning is stronger than a centroid heuristic: remove
    # the actual triangle hit in front of each P09 first-hit ray, then repeat.
    ...
    for _iteration in range(20):
        ...
        hit = scene.cast_rays(o3d.core.Tensor(np.hstack([np.zeros((len(rays), 3), np.float32), rays])))
        ...
        front_hit = valid_hit & (hit_z < p09_camera[:, 2] - float(front_tolerance_m))
    ...
    # Verify the final ray-wise contract on the P09 samples.
    ...
    if len(current_faces):
        ...
        hit_z = scene.cast_rays(o3d.core.Tensor(np.hstack([np.zeros((len(rays), 3), np.float32), rays])))['t_hit'].numpy()
...
def main() -> None:
    ...
    parser.add_argument('--annotations',type=Path,required=True);parser.add_argument('--pose-report',type=Path,required=True);parser.add_argument('--generated-mesh',type=Path,required=True);parser.add_argument('--mano-bridge',type=Path,required=True);parser.add_argument('--mano-topology',type=Path,required=True);parser.add_argument('--source-video',type=Path,required=True);parser.add_argument('--object-id',default='carton_milk');parser.add_argument('--frame-start',type=int,default=0);parser.add_argument('--frame-end',type=int,default=149);parser.add_argument('--output-rrd',type=Path,required=True);parser.add_argument('--output-video',type=Path,required=True);parser.add_argument('--decimated-face-count',type=int,default=30000);parser.add_argument('--depth-neighbor-px',type=float,default=12.0);parser.add_argument('--front-tolerance-mm',type=float,default=3.0);parser.add_argument('--point-stride',type=int,default=1);parser.add_argument('--overlay-point-stride',type=int,default=8);parser.add_argument('--jpeg-quality',type=int,default=84);parser.add_argument('--label',default='v20_depth_ordered_focused')
    ...
    report={'schema':'v20_depth_ordered_focused_visualization_v1','status':'ok','diagnostic_only':True,'formal_state_modified':False,'display_only_depth_gate':True,'pose_report':str(args.pose_report.expanduser().resolve()),'generated_mesh_role':'render_only_completion_hypothesis','depth_gate':{'neighbor_px':args.depth_neighbor_px,'front_tolerance_mm':args.front_tolerance_mm,'policy':'drop projected generated faces whose centroid is near a P09 first-hit ray and in front beyond tolerance; no pose/state changes'},'timeline':{'frame_count':len(selected),'metric_frames':metric,'no_metric_frames':empty},'stats':stats,'outputs':{'rrd':str(args.output_rrd.expanduser().resolve()),'video':str(args.output_video.expanduser().resolve())},'inputs_sha256':{'annotations':sha256_file(args.annotations),'pose_report':sha256_file(args.pose_report),'generated_mesh':sha256_file(args.generated_mesh),'mano_bridge':sha256_file(args.mano_bridge),'source_video':sha256_file(args.source_video)},'claim_scope':'Visualization-only depth ordering. Generated mesh is never pose/collision/contact/SDF authority; P09 points and MANO remain prediction-side.'}
