...
def build_case(case: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    for frame in frames:
        ...
        for side in SIDES:
            ...
            vertices_camera = (R_h.T @ (vertices_world_h - t_h[None, :]).T).T
            joints_camera = (R_h.T @ (joints_world_h - t_h[None, :]).T).T
            if vertices_camera.shape != (EXPECTED_VERTICES, 3) or joints_camera.shape != (EXPECTED_JOINTS, 3) or np.any(vertices_camera[:, 2] <= 1e-6) or np.any(joints_camera[:, 2] <= 1e-6):
    ...
    if frame_indices:
        np.savez_compressed(
            ...
            coordinate_status=np.asarray(["hawor_camera_local_transformed_by_current_v18_T_world_camera_metric_candidate_only"]),
        )
