...
User-approved physical target hypothesis: keyboard body only. Re-confirm the target and rigid branch from the raw video and your own P05/P07 visual evidence; exclude hands, table, phone, and other tabletop items. Continue with uncertainty if measurements are weak, and do not treat this hint as a pixel mask or ground truth label."
...
printf 'runtime_session=%s\nruntime_window=v19_runtime\nbundle=%s\ninput=%s\nrun_root=%s\ncase_id=%s\nmodel=%s/%s\nlog=%s\nsentinel=%s\n' "$SESSION" "$BUNDLE" "$INPUT_VIDEO" "$RUN_ROOT" "$CASE_ID" "$MODEL_PROVIDER" "$MODEL_ID" "$LAUNCH_LOG" "$DONE"
