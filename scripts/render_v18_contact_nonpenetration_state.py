#!/usr/bin/env python3
"""Render contact-owner selections with local nonpenetration evidence.

This exposes the physical-contact failure mode: the contact graph selects many
hand-object owners, but local signed/triangle evidence usually flags penetration
and therefore vetoes final accepted contact ownership. The render is evidence,
not a solved contact/nonpenetration claim.
"""
...
def contact_index(path: Path) -> tuple[dict[tuple[int, str], dict[str, Any]], dict[str, Any]]:
    ...
    for hand_graph in report.get("hand_graphs", []) if isinstance(report.get("hand_graphs"), list) else []:
        ...
        for row in hand_graph.get("assignments", []):
            ...
            if isinstance(owner, str) and owner.startswith("object:"):
                idx[(int(row.get("frame_idx", -1)), str(row.get("hand_side")))] = row
    return idx, report


def row_index(path: Path) -> dict[tuple[int, str, str], dict[str, Any]]:
    report = load_json(path)
...
def contact_status(row: dict[str, Any], signed: dict[str, Any] | None, tri: dict[str, Any] | None) -> tuple[str, tuple[int, int, int]]:
    if not row.get("accepted_contact_owner"):
        return "graph_selected_not_accepted", (255, 170, 60)
...
def render_case(case: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    contacts, contact_report = contact_index(args.contact_root / case / "v18_contact_ownership_graph_report.json")
    signed_idx = row_index(args.signed_root / case / "v18_signed_nonpenetration_evidence_report.json")
    tri_idx = row_index(args.triangle_root / case / "v18_triangle_nonpenetration_evidence_report.json")
    case_dir = args.output_root / case / "contact_nonpenetration_state"
    ...
    for raw_frame in frames:
        ...
        draw.text((12, 11), f"V18 contact/nonpenetration state {case} frame {frame_idx+1}/{len(frames)}", fill=(255, 255, 255), font=big)
        hands = hand_by_side(frame)
        objects = object_by_id(frame)
        frame_rows = [row for (idx, _side), row in contacts.items() if idx == frame_idx]
        if not frame_rows:
            counts["frames_without_selected_contact"] += 1
        y_note = image.size[1] - 24
        for row in frame_rows:
            side = str(row.get("hand_side"))
            oid = str(row.get("chosen_owner_object_id"))
            signed = signed_idx.get((frame_idx, side, oid))
            tri = tri_idx.get((frame_idx, side, oid))
            status, color = contact_status(row, signed, tri)
            hand = hands.get(side, {})
            obj = objects.get(oid, {})
            ...
            if obox is not None:
                ...
                draw.rectangle(io, outline=(80, 210, 255), width=3)
            ...
            dist = row.get("min_hand_surface_to_object_mesh_m")
            label = f"{side} contact->{str(obj.get('name') or oid).replace('object:', '')} {status} d={finite_float(dist, float('nan')):.3f}m"
            draw_label(draw, (min(hp[0], op[0]) + 8, max(52, min(hp[1], op[1]) - 22)), label[:120], small, color, (0, 0, 0))
            if signed is not None or tri is not None:
                signed_min = signed.get("min_local_signed_distance_m") if signed else None
                tri_min = tri.get("min_local_triangle_signed_distance_m") if tri else None
                note = f"local evidence: signed_min={finite_float(signed_min, float('nan')):.3f}m tri_min={finite_float(tri_min, float('nan')):.3f}m complete_SDF=false"
                draw_label(draw, (12, y_note), note[:150], small, color, (0, 0, 0))
                y_note -= 19
            counts[f"contact::{status}"] += 1
            counts[f"contact_object::{oid}"] += 1
        image.save(frame_dir / f"{frame_idx:06d}.jpg", quality=90)
    video_path = case_dir / "v18_contact_nonpenetration_state.mp4"
    ...
    if corrective_overlay.exists():
        side_path = case_dir / "v18_contact_nonpenetration_side_by_side.mp4"
        compose_side_by_side(corrective_overlay, video_path, side_path)
    report = {
        "method": "render_v18_contact_nonpenetration_state",
        "case": case,
        "claim_scope": "renders_contact_graph_selection_and_local_signed_triangle_nonpenetration_evidence; not_complete_nonpenetration_or_accepted_contact_solution",
    ...
    write_json(case_dir / "v18_contact_nonpenetration_state_report.json", report)
    return report
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        "method": "render_v18_contact_nonpenetration_state",
        "status": "contact_nonpenetration_state_render_not_complete_physical_solution",
        "cases": reports,
        "all_video_frame_counts_match": all(r["frame_counts"].get("video") == r["frame_count"] for r in reports),
        "elapsed_s": time.perf_counter() - start,
    }
    write_json(args.output_root / "v18_contact_nonpenetration_state_summary.json", summary)
    return summary
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--signed-root", type=Path, default=Path("/data2/ego_annotation_outputs/v18_signed_nonpenetration_evidence"))
