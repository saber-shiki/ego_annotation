...
P13_OUT=$EXP/p13_controlled_geometry_prior_ab_retry1
...
"$REPO/.venv/bin/python" \
  ...
  > "$EXP/p13_controlled_geometry_prior_ab_retry1_stdout.txt" \
  2> "$EXP/p13_controlled_geometry_prior_ab_retry1_stderr.txt"
code=$?
set -e
printf '%s\n' "$code" > "$EXP/p13_controlled_geometry_prior_ab_retry1_exit_code.txt"
if [ "$code" -eq 0 ]; then
  touch "$EXP/P13_CONTROLLED_AB_RETRY1_DONE"
else
  touch "$EXP/P13_CONTROLLED_AB_RETRY1_FAILED"
  tail -100 "$EXP/p13_controlled_geometry_prior_ab_retry1_stderr.txt"
fi
exit "$code"
