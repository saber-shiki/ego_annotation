...
- A multiview mask-support filter was then applied to the fused mesh. Requiring five of six graph frames to support a mesh face removes most of the rendered halo and improves projection QC to 0.823 median IoU, 0.981 median projected-vertex mask fraction, and 13.6 mm median vertex-depth absolute error. The sampled observed-to-mesh surface p95 stays low at 15.1 mm. The graph keeps plausible contact and motion with median contact p95 4.52 mm and median object center speed 0.225 m/s. The mesh remains open, so a conservative voxel SDF diagnostic was added for contact penetration. At 3 mm voxel pitch, 96.9 percent of the contact-patch points lie within 6 mm of the surface, but 62.5 percent are one voxel inside the occupied volume. This is shallow but systematic penetration, not physical closure. Reports are `/data2/ego_annotation_outputs/representative_trash/v3_observed_prior_fused_mesh_mask_supported_strict_865_870/qc_mask_supported_mesh_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_contact_patch_object_pose_graph_observed_prior_fused_masksupport_strict_865_870/qc_contact_patch_object_pose_graph_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_contact_patch_object_pose_graph_observed_prior_fused_masksupport_strict_865_870_projection_qc/qc_bundlesdf_projection_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_contact_patch_object_pose_graph_observed_prior_fused_masksupport_strict_865_870/complete_mesh_surface_conflict_qc.json`, and `/data2/ego_annotation_outputs/representative_trash/v3_contact_patch_object_pose_graph_observed_prior_fused_masksupport_strict_865_870/volume_sdf_contact_qc.json`.
- The metric-side evidence was rendered in updated review videos so the evidence can be inspected instead of inferred from metrics. The 23-frame overlay video covers frames 858 to 880 at 30 fps and draws the object mask, sparse mesh projection, MANO hand skeletons, and contact patch only on the verified contact interval: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_side_metric_858_880/mesh_surface_contact_review.mp4`. The full-window world-coordinate presentation renders all 23 frames as overlay plus third-person metric-world reconstruction, with the current head-camera frustum, measured MANO hand, object mesh, and semantic caption: `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_side_metric_full_858_880/world_reconstruction_side_by_side.mp4`. Structural QC confirms 23 frames for both full-window videos. Visual QC accepts frame 868 and frame 870 as readable contact evidence, and accepts frame 880 as an honest no-contact frame despite visible proximity. A separate 7-frame contact closeup is available at `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_side_metric_858_880/mesh_surface_contact_side_by_side.mp4`.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the metric-side MANO branch now supplies a verified mesh-surface contact interval on frames 864 to 870 at that scale. Rendering confirms that the interval is visually intelligible. The contact-patch object pose graph narrows the remaining blocker: V3 needs a complete mesh whose visible surface matches metric depth, not only a complete mesh whose silhouette and local contact patch look plausible. The observed-surface fusion and multiview mask-support branch repairs much of the visible-surface mismatch and removes most of the projection halo, but the physical contact state still has shallow voxel-SDF penetration. V3 remains open because the object geometry now needs either nonpenetrating contact optimization against the fused SDF or a watertight fused-mesh completion that preserves the same visible-surface evidence, the later visible interaction on frames 879 to 880 is only proximity evidence under the stricter metric-depth hand stream, and object mesh scale remains supported by RGB metric-depth evidence rather than a calibrated sensor or ground truth.
