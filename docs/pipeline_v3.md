...
Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. The next mesh branch must make scale ownership explicit: DROID trajectory scale, VGGT scene scale, MANO hand-size scale, object silhouette scale, and any real calibration or known-size evidence must be graph variables or factors. The graph should use VGGT surfaces as visible-patch factors and reject Depth Anything on the late frames where it contradicts VGGT by hundreds of millimeters.
