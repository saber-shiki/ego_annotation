...
Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the metric-side MANO branch now supplies a verified mesh-surface contact interval on frames 864 to 870 at that scale. Rendering confirms that the interval is visually intelligible. The contact-patch object pose graph narrows the remaining blocker: V3 needs a complete mesh whose visible surface matches metric depth, not only a complete mesh whose silhouette and local contact patch look plausible. The observed-surface fusion and multiview mask-support branch repairs much of the visible-surface mismatch, removes most of the projection halo, and supports nonpenetrating contact under a conservative volume-SDF tolerance. V3 remains open because the object deliverable still needs either watertight fused-mesh completion or an explicit SDF-backed physical consistency report attached to the rendered mesh, the later visible interaction on frames 879 to 880 is only proximity evidence under the stricter metric-depth hand stream, and object mesh scale remains supported by RGB metric-depth evidence rather than a calibrated sensor or ground truth.
