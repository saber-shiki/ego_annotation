...
- A second anatomical diagnostic measures temporal drift relative to each finger region's distal MANO joint. This is more appropriate under side/canonical changes because the contact patch should move with the finger segment, not with a frame-specific global hand coordinate origin. With strict detector score threshold 0.50, it finds ten reliable temporal contact rows on frames 862, 864, 865, 866, 867, 868, 869, 870, 879, and 880. Over reliable rows, median reprojection is 3.59 px, median MANO-minus-UniDepth depth is -11.5 mm, median best-patch p95 surface distance is 0.76 mm, median signed p95 absolute distance is 0.67 mm, and penetration fraction remains 0. The report is `/data2/ego_annotation_outputs/representative_trash/v3_wilor_track_pose_refit_wilorconv_scale055_mesh_surface_contact_anatomical_anchorrel_858_880.json`.
- The late visible interaction on frames 879 to 880 is now a diagnostic contact candidate: frame 879 is left-labeled and frame 880 is right-labeled, but both pass through the same ring-region anchor-relative contact with 13.9 mm relative drift, sub-millimeter surface distance, and zero penetration. The result still does not close V3 because the side label is wrong relative to the visible hand in frame 880, and frame 880 has -28.8 mm median MANO-minus-UniDepth depth bias. The next hand-side mechanism is therefore canonical side/track continuity and temporal MANO state ownership, not another object mask or a looser contact threshold.
- The contact evidence was rendered in updated review videos so the evidence can be inspected instead of inferred from metrics. The 23-frame overlay video covers frames 858 to 880 at 30 fps and draws the object mask, sparse mesh projection, MANO hand skeletons, and contact patch where the diagnostic accepts it: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_anatomical_anchorrel_858_880/mesh_surface_contact_review.mp4`. The world-coordinate presentation renders all 23 frames as overlay plus third-person metric-world reconstruction, with the current head-camera frustum, camera path, measured MANO hand, object mesh, and semantic caption: `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_anatomical_anchorrel_858_880/world_reconstruction_side_by_side.mp4`. Structural QC confirms 23 frames for both full-window videos. Visual QC accepts frames 868, 879, and 880 as readable diagnostic evidence. This remains evidence for contact candidates, not an annotation-ready full clip.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the WiLoR-convention hand-refit branch now supplies mesh-surface contact candidates at that scale, including a late 879 to 880 ring-contact candidate. Rendering confirms that the candidates are visually intelligible, but V3 remains open because side/canonical hand ownership and the frame-880 depth bias are unresolved. The remaining V3 work is to make the measured hand track temporally continuous in MANO's canonical frame and keep calibrated intrinsics or measured scene/object scale as unresolved metric-scale evidence.
