...
- Depth Pro was tested as an independent RGB-only metric depth and focal source on the same frames. It predicts source focal around 1500 px and mask depth around 0.40 to 0.50 m, giving apparent visible-lid widths around 0.21 to 0.26 m. Adding Depth Pro to the same static complete-mesh sweep selects scale 0.55, with a 208 x 222 x 72 mm robust mesh extent, 7.0 mm visible-patch residual, and 25 mm median absolute Depth Pro residual. That scale still leaves about 596 mm median absolute Depth Anything residual. Depth Pro therefore supplies a useful independent metric signal, but it contradicts both Depth Anything and the VGGT/DROID Sim3 scale instead of closing the object-scale problem. The Depth Pro report is `/data2/ego_annotation_outputs/representative_trash/v3_depthpro_metric_source_878_880/qc_depthpro_metric_source_v3.json`.
