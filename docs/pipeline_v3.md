...
- A per-frame measured-surface branch then corrected the depth and intrinsics contract: the observed mesh exporter now consumes UniDepth NPZ depth and annotation-VGGT intrinsics instead of the stale BundleSDF PNG depth and 2304 px dataset focal. The open measured surface has sub-millimeter z-buffer depth error but covers only about 72 percent of the full SAMWISE object mask. Per-frame solidification closes each measured surface as a 1 mm watertight sheet with zero boundary and non-manifold edges, z-buffer p95 1.46 mm, six reliable recomputed mesh-surface contact rows, selected-contact volume-SDF p95 1.25 mm at 1 mm SDF pitch, selected-contact penetration 0 percent, and full-hand penetration 0 percent. The same archive still has only 0.55 silhouette IoU against the full object mask. Reports are `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_mask_mesh_world_865_870/qc_mask_depth_observed_mesh_archive_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_mask_mesh_zbuffer_qc_865_870/qc_mesh_zbuffer_projection_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_solidified_perframe_thick001_865_870/qc_solidify_sheet_mesh_archive_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_solidified_perframe_thick001_zbuffer_qc_865_870/qc_mesh_zbuffer_projection_v3.json`, `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_solidified_perframe_thick001_865_870/mesh_surface_contact_recomputed.json`, `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_solidified_perframe_thick001_865_870/volume_sdf_contact_recomputed_pitch001_qc.json`, and `/data2/ego_annotation_outputs/representative_trash/v3_observed_unidepth_vggtK_solidified_perframe_thick001_865_870/full_hand_sdf_penetration_recomputed_pitch001_qc.json`. This branch proves that the measured visible/contact surface can satisfy depth, topology, and local hand-object physics when evaluated at a resolution compatible with the shell thickness. It is still not a complete object annotation because the broad SAMWISE object mask contains additional visible area that the measured sheet archive does not cover.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the metric-side MANO branch supplies a verified mesh-surface contact interval on frames 865 to 870 at that scale. The per-frame UniDepth/VGGT measured-sheet result separates the current blockers: local contact physics and full-hand nonpenetration are solvable on the measured visible sheet, while complete object coverage is still missing. V3 now needs a category-agnostic object completion/fusion step that expands the measured watertight contact sheet to the rest of the model-produced object mask without losing the verified local depth/contact constraints. The later visible interaction on frames 879 to 880 remains proximity evidence under the stricter metric-depth hand stream, and object mesh scale remains supported by RGB metric-depth evidence rather than a calibrated sensor or ground truth. V3 remains open.
