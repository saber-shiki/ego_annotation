...
- A static complete-mesh scale sweep then anchored one TRELLIS mesh to the VGGT visible patch at frame 880 and projected the same world mesh through frames 878 to 880 while varying the VGGT metric scale from 0.129 to 1.6. This directly tests whether a single global scale can reconcile complete mesh, visible patch, mask silhouette, and depth. It cannot. Median silhouette IoU is almost flat, about 0.70 to 0.71, and projected vertex-in-mask fraction stays around 0.89 across the entire scale range, so the silhouette cannot identify metric scale in this anchored setup. The visible-patch optimum is the Sim3 scale 0.129, which gives 1.8 mm median observed-to-prior residual but shrinks the mesh to a 49 x 52 x 17 mm robust extent and leaves 0.91 m median absolute depth error. The best silhouette scale is 0.9, with 338 x 364 x 118 mm robust extent, 12 mm median patch residual, and 0.33 m median absolute depth error. The lowest depth-error scale in the sweep is around 1.4, but it expands the robust extent to 527 x 568 x 182 mm and still leaves about 70 mm median absolute depth error. The report is `/data2/ego_annotation_outputs/representative_trash/v3_vggt_scale_static_mesh_sweep_878_880/qc_scale_static_mesh_sweep_v3.json`.
- Depth Pro was tested as an independent RGB-only metric depth and focal source on the same frames. It predicts source focal around 1500 px and mask depth around 0.40 to 0.50 m, giving apparent visible-lid widths around 0.21 to 0.26 m. This is a third scale hypothesis: shallower than VGGT's 0.69 to 0.75 m unscaled patch depth and far shallower than Depth Anything's 0.99 to 1.12 m late-frame depth. The Depth Pro review stills are visually smooth on the lid region, but the resulting object scale is smaller than the VGGT silhouette-scale estimate and larger than the Sim3-collapsed VGGT patch. The report is `/data2/ego_annotation_outputs/representative_trash/v3_depthpro_metric_source_878_880/qc_depthpro_metric_source_v3.json`.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro adds an independent focal/depth source, but it disagrees with both VGGT and Depth Anything rather than resolving scale. The next mesh branch must model depth-source reliability explicitly and add a metric-scale source that survives cross-checks: calibrated intrinsics plus a measured object/scene size, a stronger metric hand-size prior tied to MANO, or a multi-view object reconstruction whose complete geometry is constrained by more than the central visible patch.
...
1. Recover full object scale and pose for the central pink-lid object with explicit scale ownership. Use the VGGT patch as a visible-surface factor, but add full-object evidence from the SAMWISE silhouette, the TRELLIS complete-prior silhouette, MANO hand-size scale, Depth Pro/Depth Anything/VGGT depth-source reliability, and any accepted scene support or calibration source. The optimizer must keep the complete mesh at a physically plausible object scale; shrinking the complete prior to the 4 to 6 cm DROID-aligned VGGT patch is rejected.
