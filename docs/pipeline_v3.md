...
Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the same coarse scale, but that scale still disagrees with the manifest depth source and cannot be tested through contact while the hand stream contains mostly predicted or unassociated rows. The next branch must improve the measured MANO hand observation stage and keep depth-source reliability explicit; calibrated intrinsics plus a measured object/scene size would still be needed to turn the scale-0.55 hypothesis into a metric annotation.
