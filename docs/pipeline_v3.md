...
- The corrected MANO articulation refit uses WiLoR's MANO wrapper, rotation matrices with `pose2rot=False`, the saved right-hand MANO convention, explicit side x-sign mirroring, and zero-state vertex/joint checks before optimization. On the track-aware WiLoR refit stream it reaches 3.7 px median measured-hand reprojection, -6.2 mm median MANO-minus-UniDepth depth residual, 3.9 mm median contact gap, and five reliable temporal mesh-surface contact rows on frames 862, 864, 865, 866, and 867. Median best-patch p95 surface distance over measured high-score rows is 0.69 mm, median signed p95 absolute distance is 0.44 mm, and penetration fraction remains 0. The diagnostic remains open because pose deltas still frequently hit the 0.45 rad bound, and late frames 878 to 880 are visually plausible but fail compact temporal contact: frame 878 and 880 patches are slightly too spatially spread, while frame 879 is a singleton geometry row. The corrected reports are `/data2/ego_annotation_outputs/representative_trash/v3_wilor_track_pose_refit_wilorconv_scale055_unidepth_858_880/qc_pose_refit.json` and `/data2/ego_annotation_outputs/representative_trash/v3_wilor_track_pose_refit_wilorconv_scale055_mesh_surface_contact_858_880.json`.
- The contact evidence was rendered in updated review videos so the evidence can be inspected instead of inferred from metrics. The 23-frame overlay video covers frames 858 to 880 at 30 fps and draws the object mask, sparse mesh projection, MANO hand skeletons, and contact patch where the diagnostic accepts it: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_wilorconv_858_880/mesh_surface_contact_review.mp4`. The world-coordinate presentation renders all 23 frames as overlay plus third-person metric-world reconstruction, with the current head-camera frustum, camera path, measured MANO hand, object mesh, and semantic caption: `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_wilorconv_858_880/world_reconstruction_side_by_side.mp4`. Structural QC confirms 23 frames for both full-window videos. Visual QC accepts frames 866 and 879 as readable diagnostic evidence: the overlay shows the hand at the lid rim, and the world panel shows object surface, head-camera frustum, measured hand skeleton, and the accepted contact patch when present. This remains evidence for a short accepted contact episode, not an annotation-ready full clip.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the WiLoR-convention hand-refit branch now supplies a short mesh-surface contact episode at that scale. Rendering confirms that the episode is visually intelligible, but the accepted contact support is still a short episode over frames 862 and 864 to 867, with late frames 878 to 880 unresolved. The remaining V3 work is to extend the same contact proof where measurements support it and keep calibrated intrinsics or measured scene/object scale as unresolved metric-scale evidence.
