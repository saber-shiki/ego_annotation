...
- The dense measured-sheet slice now has inspected deliverable renders. The overlay video is `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_dense_measured_sheet_865_870/mesh_surface_contact_review.mp4`. The side-by-side annotated video plus world reconstruction is `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_dense_measured_sheet_finalvis_865_870/world_reconstruction_side_by_side.mp4`. The standalone world-coordinate 3D animation is `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_dense_measured_sheet_finalvis_865_870/world_reconstruction_3d.mp4`. Each video has six frames, 2 fps, and one frame per source frame from 865 to 870. Visual inspection accepts the overlay and final world view as a local presentation slice: the MANO hand, object mesh, contact markers, head camera frustum/path, metric scale, axis triad, and semantic caption are visible. The weakest contact row is frame 865, with surface and signed p95 both 5.5 mm; the other five frames are below 1.2 mm surface p95.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the metric-side MANO branch supplies a verified mesh-surface contact interval on frames 865 to 870 at that scale. The dense per-frame UniDepth/VGGT measured-sheet result now satisfies the local six-frame object-mesh, visible-depth, MANO-contact, full-hand nonpenetration, and deliverable-render checks. V3 still needs repetition on longer and more varied representative clips before closure.
