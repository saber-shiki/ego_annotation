...
- A mesh-surface contact diagnostic then tested the WiLoR-refit hands against the actual scale-0.55 object mesh instead of SAM2 surface-name fragments. It queries nearest object-mesh surface vertices in the current camera frame, estimates signed separation from mesh normals oriented toward the camera, and requires 2D reprojection, UniDepth hand depth, bone scale, patch spread, penetration fraction, and MANO-local temporal continuity to agree. It finds a three-frame right-hand contact episode on frames 866 to 868: median reprojection is 5.5 px, median MANO-minus-UniDepth depth is 1.4 mm, p95 mesh-surface distance is 0.71 mm at the median row, signed-gap p95 is 0.57 mm at the median row, and penetration fraction is 0. The report is `/data2/ego_annotation_outputs/representative_trash/v3_wilor_refit_scale055_mesh_surface_contact_858_880.json`. This is valid contact evidence for the scale-0.55 mesh hypothesis over a short window, not full V3 closure across the clip.
- The contact episode was rendered in three ways so the evidence can be inspected instead of inferred from metrics. The 23-frame overlay video covers frames 858 to 880 at 30 fps and draws the object mask, sparse mesh projection, MANO hand skeletons, and contact patch where the diagnostic accepts it: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_858_880/mesh_surface_contact_review.mp4`. The local 3D contact view renders only the accepted frames 866 to 868 and pairs the overlay with a close object-centered 3D view in camera coordinates: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_3d_review_858_880/mesh_surface_contact_side_by_side.mp4`. The world-coordinate presentation renders all 23 frames as overlay plus third-person metric-world reconstruction, with the current head-camera frustum, camera path, measured MANO hand, object mesh, and semantic caption: `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_858_880/world_reconstruction_side_by_side.mp4`. Structural QC confirms 23 frames for both full-window videos and 3 frames for the contact close-up. Visual QC accepts frame 868 as readable contact evidence: the overlay shows the right hand at the lid rim, and the world panel now shows object surface, head-camera frustum, measured hand skeleton, and contact patch. This remains evidence for the accepted contact episode, not an annotation-ready full clip.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the WiLoR-refit branch now supplies a short mesh-surface contact episode at that scale. Rendering confirms that the episode is visually intelligible, but the accepted contact support is still only frames 866 to 868. The remaining V3 work is to extend the same contact proof beyond three frames where measurements support it and keep calibrated intrinsics or measured scene/object scale as unresolved metric-scale evidence.
