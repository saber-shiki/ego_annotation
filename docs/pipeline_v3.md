...
- The mesh-surface contact diagnostic was then extended from one global nearest-vertex patch to two category-agnostic patch hypotheses: a compact global hand patch and a compact MANO anatomical patch. The anatomical patch partitions MANO vertices by nearest local hand joint, then requires the same track and same anatomical region to have temporal support. This is a dexterous-hand prior, not an object-family rule. The updated diagnostic finds six reliable temporal contact rows on frames 862, 864, 865, 866, 867, and 868. Over reliable rows, median reprojection is 3.27 px, median MANO-minus-UniDepth depth is -11.5 mm, median best-patch p95 surface distance is 0.87 mm, median signed p95 absolute distance is 0.81 mm, and penetration fraction remains 0. The report is `/data2/ego_annotation_outputs/representative_trash/v3_wilor_track_pose_refit_wilorconv_scale055_mesh_surface_contact_anatomical_858_880.json`.
- The late visible interaction on frames 878 to 880 still does not pass temporal contact. The failure is no longer a broad object-mask or patch-spread issue: each row has sub-millimeter anatomical patch candidates, but the same-region MANO-local centers jump by about 18 cm across frames because the tracked hand crosses a side-label/canonical-hand discontinuity. Frame 880 also has -28.8 mm median MANO-minus-UniDepth depth bias. The next hand-side mechanism is therefore canonical side/track continuity and temporal MANO state ownership, not another object mask or a looser contact threshold.
- The contact evidence was rendered in updated review videos so the evidence can be inspected instead of inferred from metrics. The 23-frame overlay video covers frames 858 to 880 at 30 fps and draws the object mask, sparse mesh projection, MANO hand skeletons, and contact patch where the diagnostic accepts it: `/data2/ego_annotation_outputs/representative_trash/v3_mesh_surface_contact_review_anatomical_858_880/mesh_surface_contact_review.mp4`. The world-coordinate presentation renders all 23 frames as overlay plus third-person metric-world reconstruction, with the current head-camera frustum, camera path, measured MANO hand, object mesh, and semantic caption: `/data2/ego_annotation_outputs/representative_trash/v3_world_reconstruction_review_anatomical_858_880/world_reconstruction_side_by_side.mp4`. Structural QC confirms 23 frames for both full-window videos. Visual QC accepts frame 868 as readable diagnostic evidence, while frame 880 correctly labels no reliable mesh-surface contact. This remains evidence for a short accepted contact episode, not an annotation-ready full clip.

Interpretation:

Object-complete asset priors are feasible, but the current object crops and the central VGGT visible patch underconstrain hidden geometry and absolute scale. Mesh alignment alone can fit the partial observed surface while inventing or shrinking the object. A single global VGGT scale is also insufficient because the anchored projection can preserve silhouette while metric size and depth move by orders of magnitude. Depth Pro and UniDepth independently favor the 0.55 scale, and the WiLoR-convention hand-refit branch now supplies a short mesh-surface contact episode at that scale. Rendering confirms that the episode is visually intelligible, but the accepted contact support is still a short episode over frames 862 and 864 to 868, with late frames 878 to 880 unresolved. The remaining V3 work is to make the measured hand track temporally continuous in MANO's canonical frame and keep calibrated intrinsics or measured scene/object scale as unresolved metric-scale evidence.
