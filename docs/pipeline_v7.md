...
The 702 to 708 remote A800 measurement job is queued under tmux session `ego_v7_mop_702_708_wait`. It runs the remote measurement chain: VLM point prompts, SAM2 object and hand masks, UniDepth metric depth, VGGT camera, HaMeR projection measurement, MANO mask/depth articulation refit, hand hypothesis selection, observed mesh export, CoTracker tracks, sparse track edges, and pairwise rigid factors. The queued script now fails immediately if hand selection does not produce one measured MANO hand in every requested frame. After the remote measurement report exists, `scripts/sync_and_run_v7_mop_702_708_acceptance.sh` syncs the outputs locally, rewrites remote paths, runs replay QC, topology-aware track QC, physics QC, and renders the final overlay, 3D world, and side-by-side deliverables. The tmux job is waiting for an A800 GPU below the memory threshold because all GPUs are currently occupied by another user's active training process.
