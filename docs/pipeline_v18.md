...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_local_surface_contact.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, and local surface-changing contact-support repair: trash has 8 active deformable visible-surface contacts, 5 depth-occluded possible-contact rows, 5 matching contact-depth occlusion evidence rows, and 33 supported-near noncontact rows; task5 now has 2 active left-hand tomato contacts at frames 926 and 927, 0 depth-occluded possible rows, 1 supported-near faucet-handle noncontact row, and 1 raw unsupported faucet proposal. Frame 926 uses full surface-changing visible-pose support with 2.76 cm MANO/object distance; frame 927 uses local observed-surface support under partial visibility with 5.92 cm MANO/object distance, observed mask support 0.875, observed-to-predicted median residual 6.69 cm, mesh support 0.995, observed HaWoR, and no nonpenetration conflict. Frame-local part validation covers all final part-validation rows (`trash_1050`: 332/605 supported; `task5_tomato_960`: 21/100 supported), preventing faucet/off-white contact modes from borrowing aggregate track support on locally rejected frames. The task5 frame-937 faucet row reports the supported handle distance (10.48 cm), not the rejected lever's nearer diagnostic distance. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`); `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
