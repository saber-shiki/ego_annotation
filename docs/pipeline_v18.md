...
- Active contacts with depth contradiction are blocked unless the contradiction is explicitly demoted by a bounded visual-contact prior: image contact evidence, high mesh contact support, close MANO/object geometry (<=7 cm), observed HaWoR, no nonpenetration conflict, and a final post-graph object/part support path. For partially visible surface-changing objects, the final support path may be a local observed-surface contact support (`surface_changing_local_visible_contact_surface`) when observed surface projection remains inside the current mask and residuals are bounded; this supports contact only and does not complete hidden geometry or full object pose. Without full or local support, a depth-contradicted row may render only as `depth_occluded_contact_possible` when final support and near geometry exist, or remain raw/contradicted evidence. Depth-occluded rows are non-active uncertainty states, not contact claims, and populate local hand/object `contact_depth_order_occlusion` evidence without becoming global occlusion-owner claims.
...
- Unsupported contact proposals stay in JSON as raw/gated evidence and must not be drawn as solved or possible contact. Overlay/world videos may render active contacts and the two scoped non-active modes with different labels/styles; only `active_physical_contact` contributes to `contact_lines`, `world_contact_edges`, or active-contact counts.
- World-render active and non-active contact endpoints are driven by metric MANO/world and metric object/part/mesh/world state, not 2D bbox centers. Non-active world contact modes are skipped if metric endpoints are missing.
- Compact multiview object-geometry completion is now a separate assessment from visible-pose validation. It can set `object_geometry_complete=true` and `object_pose_requirement_met=true` only for schema-eligible compact objects with dense multiview depth-fused mesh evidence and current-frame visible depth/silhouette pose support. It remains blocked for deformable objects, part/relative-motion objects, sparse reconstructions, and frames without current visible support.

Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_compact_geometry_completion.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, and compact multiview geometry-completion assessment: trash has 8 active deformable visible-surface contacts, 5 depth-occluded possible-contact rows, 5 matching contact-depth occlusion evidence rows, 33 supported-near noncontact rows, and 0 object-geometry-complete rows; task5 has 2 active left-hand tomato contacts at frames 926 and 927, 0 depth-occluded possible rows, 1 supported-near faucet-handle noncontact row, 1 raw unsupported faucet proposal, and 10 compact multiview tomato object-geometry/object-pose rows. Frame 926 uses full surface-changing visible-pose support with 2.76 cm MANO/object distance; frame 927 uses local observed-surface support under partial visibility with 5.92 cm MANO/object distance, observed mask support 0.875, observed-to-predicted median residual 6.69 cm, mesh support 0.995, observed HaWoR, and no nonpenetration conflict. The compact tomato geometry rows require a schema-eligible compact object, 447 source frames, 21k depth points, 88 convex-hull faces, 11k Poisson vertices, and current-frame visible depth/silhouette support; they are not category primitives or centroid proxies. Frame-local part validation covers all final part-validation rows (`trash_1050`: 332/605 supported; `task5_tomato_960`: 21/100 supported), preventing faucet/off-white contact modes from borrowing aggregate track support on locally rejected frames. The task5 frame-937 faucet row reports the supported handle distance (10.48 cm), not the rejected lever's nearer diagnostic distance. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`); object geometry/pose remains unresolved for trash, faucet, tomato-peel, and task5 frames lacking current-frame visible support.
