...
- Active contacts with depth contradiction are blocked. A depth-contradicted row may render only as `depth_occluded_contact_possible` when final validated object/part/deformable support exists, same-frame MANO/geometry distance is within the near band, and the raw contact energy preferred contact before the depth blocker. This is a non-active uncertainty state, not a contact claim. The same rows also populate hand/object `contact_depth_order_occlusion` evidence, scoped to the contact pair and explicitly not an accepted global occlusion-owner claim.
- `supported_near_noncontact` is a second non-active mode for final validated object/part/deformable support within the near band when the contact switch remains off and depth ordering is not contradictory. For part-required objects, this now requires same-frame part depth/mask validation rather than aggregate track-level support, and its reported distance/endpoints come from the supported part path rather than any rejected nearer part. It records a physically close state without claiming contact.
...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_supported_distance_fix.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, and supported-distance repair: trash has 8 active deformable visible-surface contacts, 3 depth-occluded possible-contact rows, 3 matching contact-depth occlusion evidence rows, and 5 supported-near off-white-lid noncontact rows; task5 has 0 active contacts, 1 depth-occluded possible tomato row, 1 matching contact-depth occlusion evidence row, and 1 supported-near faucet-handle noncontact row. Frame-local part validation covers all final part-validation rows (`trash_1050`: 332/605 supported; `task5_tomato_960`: 21/100 supported), preventing faucet/off-white contact modes from borrowing aggregate track support on locally rejected frames. The task5 frame-937 faucet row reports the supported handle distance (10.48 cm), not the rejected lever's nearer diagnostic distance. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`); `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
