...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_complete_depth_occlusion_owner_schema.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, owner-bounded manipulation-contact episodes, strict part-contact pose coupling, validation-aware object/part rendering, complete-depth visible-surface integration, compact clean-rigid geometry completion, and complete-depth/Hawor occlusion ownership: trash has 71 active contact frame-pair states, 12 consecutive active temporal contact episodes, 70 `contact_episode` variables/factors, 23 supported-near noncontact rows, 0 `contact_part_pose_anchor` factors, 0 object-geometry/object-pose-complete rows, and 3 accepted occlusion-owner variables/edges. Task5 has 699 active contact frame-pair states, 11 consecutive active temporal contact episodes, and 699 `contact_episode` variables/factors: 571 left-hand tomato states and 128 right-hand tomato states. For task5, only 2 states are direct visible/local surface anchors, 467 are local occluded-contact-patch anchors with accepted contact-owner support, and 230 are bounded short-gap inferences; every episode-supported frame is within the configured 10-frame nearest-anchor bound. The earlier two plastic-container rows are no longer active after requiring accepted contact-owner support for occluded anchors. Task5 also has 1 supported-near faucet-handle noncontact row, 0 raw physically gated proposals, 0 `contact_part_pose_anchor` factors, 2 valid `contact_surface_changing_object_pose_anchor` factors at the direct tomato anchors, and 0 accepted global occlusion-owner variables. Frame-local part validation remains `trash_1050`: 332/605 supported and `task5_tomato_960`: 21/100 supported. Complete-depth visible surfaces support 106 clean-rigid task5 object-geometry/object-pose-complete rows: bowl 15, egg 30, and steaming rack 61. Final renders separate validation states: trash draws 1,604 rejected object-mesh candidates, 3 occlusion-owner edges, 346 supported part-pose marks, and 259 rejected part-pose marks; task5 draws 106 completed clean-rigid geometry poses, 10 supported visible object poses, 992 rejected object-mesh candidates, 62 supported part-pose marks, and 38 rejected part-pose marks. Complete object geometry/pose remains unresolved for trash, task5 tomato, faucet, tomato peel, deformable objects, and part/relative-motion objects.
