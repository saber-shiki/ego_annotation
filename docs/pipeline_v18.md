...
- Compact multiview object-geometry completion is now a separate strict clean-rigid assessment from visible-pose/contact validation. It can set `object_geometry_complete=true` and `object_pose_requirement_met=true` only for clean rigid compact objects with dense multiview depth-fused mesh evidence and current-frame visible depth/silhouette pose support. It remains blocked for surface-changing objects, deformable objects, part/relative-motion objects, sparse reconstructions, and frames without current visible support. Surface-changing tomato support can anchor contact/visible pose; it cannot complete hidden geometry or the object-pose requirement. Rendered object mesh marks are validation-aware: supported visible poses are visually separated from rejected mesh candidates, so a drawn mesh candidate is not automatically a pose or geometry-completion claim.

Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_complete_depth_default.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, owner-bounded manipulation-contact episodes, strict part-contact pose coupling, validation-aware object/part rendering, complete-depth visible-surface integration, and strict clean-rigid compact geometry eligibility: trash has 71 active contact frame-pair states, 12 consecutive active temporal contact episodes, 70 `contact_episode` variables/factors, 23 supported-near noncontact rows, 0 `contact_part_pose_anchor` factors, and 0 object-geometry/object-pose-complete rows. Task5 has 699 active contact frame-pair states, 11 consecutive active temporal contact episodes, and 699 `contact_episode` variables/factors: 571 left-hand tomato states and 128 right-hand tomato states. For task5, only 2 states are direct visible/local surface anchors, 467 are local occluded-contact-patch anchors with accepted contact-owner support, and 230 are bounded short-gap inferences; every episode-supported frame is within the configured 10-frame nearest-anchor bound. The earlier two plastic-container rows are no longer active after requiring accepted contact-owner support for occluded anchors. Task5 also has 1 supported-near faucet-handle noncontact row, 0 raw physically gated proposals, 0 `contact_part_pose_anchor` factors, 2 valid `contact_surface_changing_object_pose_anchor` factors at the direct tomato anchors, and 0 object-geometry/object-pose-complete rows. Frame-local part validation remains `trash_1050`: 332/605 supported and `task5_tomato_960`: 21/100 supported. Final renders now separate validation states: trash draws 1,604 rejected object-mesh candidates plus 346 supported and 259 rejected part-pose marks; task5 draws 116 supported visible object poses, 992 rejected object-mesh candidates, 62 supported part-pose marks, and 38 rejected part-pose marks. Complete-depth visible surfaces add task5 visible-pose support for bowl/egg/steaming-rack frames while keeping all object-geometry/object-pose-complete rows at 0. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`). Complete object geometry/pose remains unresolved for trash, task5 tomato, faucet, tomato peel, deformable objects, and part/relative-motion objects.
