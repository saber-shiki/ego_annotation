...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_contact_episode_ownerbounded_final.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, owner-bounded manipulation-contact episodes, and strict clean-rigid compact geometry eligibility: trash has 71 active contact frame-pair states, 12 consecutive active temporal contact episodes, 70 `contact_episode` variables/factors, 23 supported-near noncontact rows, and 0 object-geometry/object-pose-complete rows. Task5 has 699 active contact frame-pair states, 11 consecutive active temporal contact episodes, and 699 `contact_episode` variables/factors: 571 left-hand tomato states and 128 right-hand tomato states. For task5, only 2 states are direct visible/local surface anchors, 467 are local occluded-contact-patch anchors with accepted contact-owner support, and 230 are bounded short-gap inferences; every episode-supported frame is within the configured 10-frame nearest-anchor bound. The earlier two plastic-container rows are no longer active after requiring accepted contact-owner support for occluded anchors. Task5 also has 1 supported-near faucet-handle noncontact row, 2 raw physically gated proposals, and 0 object-geometry/object-pose-complete rows. Frame-local part validation remains `trash_1050`: 332/605 supported and `task5_tomato_960`: 21/100 supported. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`). Complete object geometry/pose remains unresolved for trash, task5 tomato, faucet, tomato peel, deformable objects, and part/relative-motion objects.
