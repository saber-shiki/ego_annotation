...
- Latest validated final counts: trash has 348 active contact states, 8 emitted stable `contact_part_pose_anchor` factors, 340 `deformable_surface_patch` variables, 34 supported-near noncontact rows, 3 accepted occlusion-owner variables, 3 observed MANO pose-fill-through-occlusion rows with matching `hand_occlusion_pose_fill` factors, and 0 object-geometry/object-pose-complete rows. Task5 has 32 active contact states, 19 emitted stable pose anchors (11 object, 8 part), 5 `deformable_surface_patch` variables, 8 direct active contacts marked not pose-coupled because the anchor fixed point oscillated, 15 supported-near noncontact rows, 0 pose-fill-through-occlusion rows, 101 compact rigid object-geometry/object-pose-complete rows, and 2 structured part-object pose-ready rows.
...
Latest validated run for this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_posefill_embedded_owner_labels.log`.
