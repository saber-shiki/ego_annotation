...
- Active contacts with depth contradiction are blocked. A depth-contradicted row may render only as `depth_occluded_contact_possible` when final validated object/part/deformable support exists, same-frame MANO/geometry distance is within the near band, and the raw contact energy preferred contact before the depth blocker. This is a non-active uncertainty state, not a contact claim.
...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_frame_local_part_pose_fix4.log`.
Current scoped effect after contact-mode and frame-local part-support repair: trash has 8 active deformable visible-surface contacts, 3 depth-occluded possible-contact rows, and 5 supported-near off-white-lid noncontact rows; task5 has 0 active contacts, 1 depth-occluded possible tomato row, and 1 supported-near faucet-handle noncontact row. Frame-local part validation covers all final part-validation rows (`trash_1050`: 332/605 supported; `task5_tomato_960`: 21/100 supported), preventing faucet/off-white contact modes from borrowing aggregate track support on locally rejected frames. `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
