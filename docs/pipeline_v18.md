...
- `supported_near_noncontact` is a second non-active mode for final validated object/part/deformable support within the near band when the contact switch remains off and depth ordering is not contradictory. It records a physically close state without claiming contact.
...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_contact_modes_fix1.log`.
Current scoped effect after contact-mode repair: trash has 8 active deformable visible-surface contacts, 3 depth-occluded possible-contact rows, and 5 supported-near noncontact rows; task5 has 0 active contacts, 1 depth-occluded possible tomato row, and 2 supported-near faucet-handle noncontact rows. `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
