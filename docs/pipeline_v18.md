...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_active_contact_coupling_state.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, owner-bounded manipulation-contact episodes, strict part-contact pose coupling, validation-aware object/part rendering, complete-depth visible-surface integration, compact clean-rigid geometry completion, complete-depth/Hawor occlusion ownership, frame-local part-pose readiness, updated pose-fill blocker evidence, structured part-object pose readiness, final contact-claim/evidence separation, episode-only contact demotion, and explicit active-contact coupling state: trash has 12 active contact frame-pair states, 5 consecutive active temporal contact episodes, 59 non-active `contact_episode_hypothesis_nonactive` rows, 23 supported-near noncontact rows, 0 `contact_part_pose_anchor` factors, 0 object-geometry/object-pose-complete rows, 3 accepted occlusion-owner variables/edges, 332 ready part-pose rows for the off-white lid, and 0 structured part-object pose rows because the hinge is not frame-ready with the lid. All 12 trash active contacts are explicitly marked `active_deformable_contact_state_not_coupled_to_object_pose` with blocker `deformable_object_contact_has_no_nonrigid_object_state_model_in_v18_default_solver`. The three accepted trash occlusion-owner frames now flow into the pose-fill gate, but no hand pose is filled because each remains blocked by missing HaWoR/interior-depth/final hand-baseline evidence. Task5 has 2 active contact frame-pair states, 1 consecutive active temporal contact episode, 697 non-active `contact_episode_hypothesis_nonactive` rows, and 1 supported-near faucet-handle noncontact row. Task5 also has 0 raw physically gated proposals, 0 `contact_part_pose_anchor` factors, 2 valid `contact_surface_changing_object_pose_anchor` factors at the direct tomato anchors, 0 accepted global occlusion-owner variables, 21 ready faucet part-pose rows, and 2 frame-local structured faucet part-object pose rows at frames 342 and 344 where both handle and lever parts are ready. The two task5 active tomato contacts are explicitly marked as affecting object pose through `contact_surface_changing_object_pose_anchor`; no active row lacks direct frame-local visible/validated support, and episode-only rows no longer carry solved `physical_contact_claim_supported=true`. Complete-depth visible surfaces support 106 clean-rigid task5 object-geometry/object-pose-complete rows: bowl 15, egg 30, and steaming rack 61. Final renders separate validation states: trash draws 1,604 rejected object-mesh candidates, 3 occlusion-owner edges, 332 ready part-pose marks, 273 rejected part-pose marks, and 0 structured part-object readiness labels; task5 draws 106 completed clean-rigid geometry poses, 10 supported visible object poses, 992 rejected object-mesh candidates, 21 ready part-pose marks, 79 rejected part-pose marks, and 2 structured part-object readiness labels. Complete object geometry/pose remains unresolved for trash, task5 tomato, faucet whole-object geometry, tomato peel, deformable objects, and other part/relative-motion objects.
