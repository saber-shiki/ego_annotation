...
Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_visual_prior_fix2.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, and bounded visual-prior repair: trash has 8 active deformable visible-surface contacts, 5 depth-occluded possible-contact rows, 5 matching contact-depth occlusion evidence rows, and 33 supported-near noncontact rows; task5 now has 1 active left-hand tomato contact at frame 926, 0 depth-occluded possible rows, 1 supported-near faucet-handle noncontact row, and 2 raw unsupported contact proposals. The active tomato row is a bounded visual-prior override of weak depth order with final surface-changing visible-pose support, 2.76 cm MANO/object distance, mesh support 0.998, observed HaWoR, and no nonpenetration conflict; adjacent frame 927 remains non-active because final post-graph support is missing. Frame-local part validation covers all final part-validation rows (`trash_1050`: 332/605 supported; `task5_tomato_960`: 21/100 supported), preventing faucet/off-white contact modes from borrowing aggregate track support on locally rejected frames. The task5 frame-937 faucet row reports the supported handle distance (10.48 cm), not the rejected lever's nearer diagnostic distance. Global occlusion-owner variables remain unsupported (`occlusion_owner_supported_vars=0`); `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
