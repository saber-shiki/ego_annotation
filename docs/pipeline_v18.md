...
## Checkpoint 2026-06-16 — metric alignment, structured schema, and stable contact-pose anchors
...
- Final contact semantics distinguish active contact, pose-coupled active contact, stable-contact-without-emitted-factor, unstable active contact, deformable active contact without a nonrigid model, and non-active evidence rows. Rendered contact lines do not redefine these states.
- Latest validated final counts: trash has 348 active contact states, 7 emitted stable `contact_part_pose_anchor` factors, 1 stable direct part-contact support row without an emitted pose factor, 34 supported-near noncontact rows, 3 accepted occlusion-owner variables, and 0 object-geometry/object-pose-complete rows. Task5 has 32 active contact states, 19 emitted stable pose anchors (11 object, 8 part), 8 direct active contacts marked not pose-coupled because the anchor fixed point oscillated, 15 supported-near noncontact rows, 101 compact rigid object-geometry/object-pose-complete rows, and 2 structured part-object pose-ready rows.
...
Latest validated run for this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_no_nonpenetration_pose_repel.log`.
