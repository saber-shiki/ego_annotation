...
- Active contacts with depth contradiction are blocked. Depth-contradicted proposals stay unresolved rather than rendering as solved contact.
- Rendered contact edges in overlay/world videos are drawn only from active physical contact switches. Unsupported contact proposals stay in JSON as raw/gated evidence and must not be drawn as solved contact. World-render object, hand, and contact endpoints are driven by metric MANO/world and metric object/mesh/world state, not 2D bbox centers.

Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260615_critic_fix5.log`.
Current scoped effect after adversarial repair: trash has 8 active deformable visible-surface contacts; task5 has 0 active contacts because the previous tomato contact rows are depth-contradicted and therefore blocked. `object_geometry_complete` and `object_pose_requirement_met` remain false across both cases.
