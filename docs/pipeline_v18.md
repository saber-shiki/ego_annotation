...
- `supported_near_noncontact` is a second non-active mode for final validated object/part/deformable support within the near band when the contact switch remains off and depth ordering is not contradictory. For part-required objects, this now requires same-frame part depth/mask validation rather than aggregate track-level support. It records a physically close state without claiming contact.
