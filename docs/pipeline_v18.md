...
`build_v18_part_visible_surfaces.py` then extracts metric-depth-backed visible surfaces for the accepted generated part masks, without OpenCV and without BundleSDF/NeRF. It writes a compact NPZ archive in depth-camera coordinates. Current totals: 809 accepted part visible-surface frame rows, 234,193 vertices, and 409,969 faces. Rejections remain scoped to missing metric depth, part/object containment failure, or too few connected sampled vertices/faces.

The updated status manifest reports `part_required_object_count=3`, `accepted_part_track_assignment_count=5`, `part_visible_surface_frame_rows=809`, `part_visible_surface_vertices=234193`, `part_visible_surface_faces=409969`, `part_pose_ready_count=0`, and `object_pose_requirement_met=false`. This advances V18 from whole-object visible surfaces to generated part-level visible surface evidence, but it still does not reconstruct hidden part geometry, estimate part pose, or validate contact ownership.
...
`build_v18_part_model_candidates.py` now records accepted candidates, rejected residual probes, and surface-level ICP diagnostics over the actual depth-backed part vertices. Under the current generated-only evidence it accepts no part model candidates and writes three rejected probes: off-white pair rejected because one part is sparse/unstable, pink-lid single-part probe rejected because a split/articulation model requires at least two semantic part tracks, and faucet pair rejected because both small-part surfaces remain sparse/unstable.

The updated status manifest reports `part_model_candidate_count=0`, `part_model_rejected_candidate_count=3`, `part_surface_icp_probe_count=5`, `part_surface_icp_probe_state_counts={surface_icp_residual_supported_visible_only_not_pose: 2, surface_icp_sparse_or_unstable_not_pose: 3}`, `visible_subset_model_candidate_count=0`, `articulation_model_ready_count=0`, `part_pose_ready_count=0`, and `object_pose_requirement_met=false`. Surface ICP is diagnostic visible-shape evidence, not part pose.
...
- `object:obj_faucet_handle`: a two-part relative-motion residual probe is rejected because adaptive sampling recovers shared frames, but the variable pair still involves sparse/unstable part surfaces.

The current summary records `part_model_candidate_count=0`, `part_model_rejected_candidate_count=3`, `part_surface_icp_probe_count=5`, `part_surface_icp_probe_state_counts={surface_icp_residual_supported_visible_only_not_pose: 2, surface_icp_sparse_or_unstable_not_pose: 3}`, `part_pose_ready_count=0`, and `object_pose_requirement_met_count=0`. `build_v18_part_object_blocker_manifest.py` now reports all three required part objects as `blocked_part_model_residual_probes_rejected`, not as missing mask evidence or accepted part models.
...
The adaptive sampler improves visible part geometry evidence but does not make any part model valid. The residual-probe stage still reports `part_model_candidate_count=0`, `part_model_rejected_candidate_count=3`, `part_pose_ready_count=0`, and `object_pose_requirement_met=false`. The faucet pair is no longer underconstrained by missing surfaces, but it remains rejected because the variable pair involves sparse/unstable surfaces.


## Implementation Checkpoint 30: Surface-Level ICP Diagnostics for Rejected Part Models

`build_v18_part_model_candidates.py` now attaches bounded surface-level ICP probes to rejected part-model candidates. Each probe aligns sampled depth-backed part-surface vertices from selected frames to a reference part surface and reports median/p95 residuals, support counts, and explicit blockers. The coordinate frame is per-frame metric-depth camera; ICP estimates only visible-shape consistency, not world pose or articulation.

Current generated-only evidence produces 5 ICP probes. Two robust single-part visible surfaces have low residuals and enough support: off-white lid and pink-lid lid. Three small parts have low alignment residuals but remain too sparse for pose/model promotion: off-white hinge, faucet handle, and faucet lever. This distinguishes the failure mechanism from a pure surface-residual failure: the next repair target is denser/more stable small-part mask/depth support and a second semantic part for pink-lid, not threshold-lowering.

No readiness is promoted. The manifest reports `part_surface_icp_probe_count=5`, `part_surface_icp_probe_state_counts={surface_icp_residual_supported_visible_only_not_pose: 2, surface_icp_sparse_or_unstable_not_pose: 3}`, `part_model_candidate_count=0`, `part_model_rejected_candidate_count=3`, `part_pose_ready_count=0`, and `object_pose_requirement_met=false`. The invariant audit now checks these fields.
