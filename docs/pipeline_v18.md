...
- Active contacts are physical frame-pair states, not rendered line counts or isolated visible-surface hits. A contact switch may be active either from direct visible/validated geometry support or from a directly anchored manipulation contact episode. The episode path is explicit graph state (`contact_episode` variables and `contact_episode_persistence` factors): it requires observed HaWoR support, no local nonpenetration conflict, continuous hand-object manipulation evidence, and at least one direct anchor frame. Episode-supported frames preserve the visible-surface distance as evidence, but that distance is not interpreted as the contact-patch gap when the contact patch is occluded or unmodeled. This supports contact only; it does not complete hidden geometry, object pose, or global occlusion ownership.
...
- Compact multiview object-geometry completion is now a separate strict clean-rigid assessment from visible-pose/contact validation. It can set `object_geometry_complete=true` and `object_pose_requirement_met=true` only for clean rigid compact objects with current-frame visible depth/silhouette pose support, at least 25 source frames, at least 5,000 fused depth points, and non-sparse hull/Poisson mesh topology. It remains blocked for surface-changing objects, deformable objects, part/relative-motion objects, sparse reconstructions, and frames without current visible support. Surface-changing tomato support can anchor contact/visible pose; it cannot complete hidden geometry or the object-pose requirement. Rendered object mesh marks are validation-aware: completed clean-rigid poses, supported visible poses, and rejected mesh candidates are visually separated.

Latest validated full run at this checkpoint: `/data2/ego_annotation_outputs/v18_full_pipeline/final_run_20260616_contact_claim_semantics.log`.
Current scoped effect after contact-mode, frame-local part-support, contact-depth occlusion, supported-distance, deformable-near noncontact, bounded visual-prior, local surface-changing contact-support, owner-bounded manipulation-contact episodes, strict part-contact pose coupling, validation-aware object/part rendering, complete-depth visible-surface integration, compact clean-rigid geometry completion, complete-depth/Hawor occlusion ownership, frame-local part-pose readiness, updated pose-fill blocker evidence, structured part-object pose readiness, and final contact-claim/evidence separation: trash has 71 active contact frame-pair states, 12 consecutive active temporal contact episodes, 70 `contact_episode` variables/factors, 23 supported-near noncontact rows, 0 `contact_part_pose_anchor` factors, 0 object-geometry/object-pose-complete rows, 3 accepted occlusion-owner variables/edges, 332 ready part-pose rows for the off-white lid, and 0 structured part-object pose rows because the hinge is not frame-ready with the lid. The three accepted trash occlusion-owner frames now flow into the pose-fill gate, but no hand pose is filled because each remains blocked by missing HaWoR/interior-depth/final hand-baseline evidence. Task5 has 699 active contact frame-pair states, 11 consecutive active temporal contact episodes, and 699 `contact_episode` variables/factors: 571 left-hand tomato states and 128 right-hand tomato states. For task5, only 2 states are direct visible/local surface anchors, 467 are local occluded-contact-patch anchors with accepted contact-owner support, and 230 are bounded short-gap inferences; every episode-supported frame is within the configured 10-frame nearest-anchor bound. Task5 also has 1 supported-near faucet-handle noncontact row, 0 raw physically gated proposals, 0 `contact_part_pose_anchor` factors, 2 valid `contact_surface_changing_object_pose_anchor` factors at the direct tomato anchors, 0 accepted global occlusion-owner variables, 21 ready faucet part-pose rows, and 2 frame-local structured faucet part-object pose rows at frames 342 and 344 where both handle and lever parts are ready. Non-active near/evidence rows no longer carry solved `physical_contact_claim_supported=true`: trash has 15 non-active evidence-supported rows and task5 has 246 non-active evidence-supported rows, all separated as evidence rather than active contact claims. Complete-depth visible surfaces support 106 clean-rigid task5 object-geometry/object-pose-complete rows: bowl 15, egg 30, and steaming rack 61. Final renders separate validation states: trash draws 1,604 rejected object-mesh candidates, 3 occlusion-owner edges, 332 ready part-pose marks, 273 rejected part-pose marks, and 0 structured part-object readiness labels; task5 draws 106 completed clean-rigid geometry poses, 10 supported visible object poses, 992 rejected object-mesh candidates, 21 ready part-pose marks, 79 rejected part-pose marks, and 2 structured part-object readiness labels. Complete object geometry/pose remains unresolved for trash, task5 tomato, faucet whole-object geometry, tomato peel, deformable objects, and other part/relative-motion objects.
