...
V17 now exports object-track datasets under `/data2/ego_annotation_outputs/v17_object_track_datasets`. These datasets contain per-object RGB, mask, metric-depth PNGs, and intrinsics at the 960 by 540 depth resolution. They are inputs for material-correspondence tracking, not tracking results. Trash exports four object datasets with 1,448 total frames and 185 rejected frames. Tomato exports four usable object datasets with 936 total frames and 213 rejected frames; five tomato context objects have no exported tracking dataset because visible masks and metric depth do not overlap enough. The next solver step must run a point tracker and test rigid or deformable motion from those tracks.

V17 now also has a first material-correspondence measurement layer under `/data2/ego_annotation_outputs/v17_object_material_tracks`. The tracker consumes the V17 object-track dataset contract directly: RGB, SAM2 object mask, depth PNG, per-frame depth intrinsics, and graph annotation camera pose. It does not read legacy single-object intrinsics fields. The early tomato window 0274-0333 produced only four query points and zero rigid-ready adjacent pairs, which shows that small visible masks cannot support pose factors. A denser tomato window 0690-0749 produced 73 query points and 9 rigid-ready adjacent-frame pairs after the rigid diagnostic required enough inliers, local spatial support, plausible centroid displacement, and plausible rotation angle. Trash now has three attempted windows: the off-white can window 0182-0241 has 25 rigid-ready adjacent pairs, the white-bag window 0720-0779 has 30 local rigid-ready adjacent pairs, and the white-bag window 0800-0859 has 10 local rigid-ready adjacent pairs despite zero tracks surviving the full 60-frame window. The generated material-track summary records five attempted windows across both clips, three tracked objects, and 74 rigid-ready adjacent pairs. Black trash bag, pink-lid trash can, faucet handle, plastic container, and tomato peel still have exported object-track datasets without material-track windows. These tracks are local correspondence evidence; they are not a full-timeline object pose state, do not reconstruct complete object geometry, and do not satisfy the V3 solver requirement.

V17 also now writes a joint solver problem contract under `/data2/ego_annotation_outputs/v17_joint_solver_problem`. That contract is a generated gap audit, not an optimizer. It binds the V3 requirement to concrete variable families and reports the still-missing families: camera trajectory, MANO articulation and shape, multi-object identity timeline, object geometry topology, object pose or deformation state, hand-object contact modes for every object pair, contact patch identity, dense depth and visible-surface state, and physical consistency terms. The contract now requires the multi-object visible-surface, geometry-state, object-track-dataset, and object-material-track reports and records their measured, rejected, envelope-candidate, rigid-candidate, exported tracking-input, material-track-window, and rigid-factor rows inside the object-geometry and object-pose variable families. The contract makes the liability explicit: the sparse graph optimizes a small consistency layer over fixed central state variables.
