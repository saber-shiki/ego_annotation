...
V17 now writes evaluated reconstruction-result reports under `/data2/ego_annotation_outputs/v17_geometry_reconstruction_results`. This layer consumes the prepared RGBD jobs and a BundleSDF output root, then checks whether each solver output has a mesh, a complete per-frame object-in-camera pose sequence, a mesh scale compatible with the rectified RGBD observations, plausible mesh topology, and projection/depth agreement against the job masks and depth maps. Missing backend output is recorded as `pending_solver_output`; a bad mesh is recorded as rejected evidence. The current local report has one rejected BundleSDF result and one pending result row. The first BundleSDF run produced all nine pose files and `mesh_cleaned.obj`, but V17 rejected the mesh because its maximum extent is 1.86 m while the rectified RGBD object-window scale supports at most 0.90 m under the current acceptance ratio. This preserves the earlier V3 failure mechanism as evidence instead of silently promoting a meter-scale mesh. The earlier scheduler failure was causal, not visual: the queue mixed GPU-wait diagnostics into the stdout value used as `CUDA_VISIBLE_DEVICES`, so PyTorch saw no CUDA devices. The tracked queue script now emits only the selected GPU id on stdout, logs diagnostics on stderr, and runs the mutating BundleSDF checkout under a file lock.
