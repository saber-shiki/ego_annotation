...
V17 now writes evaluated reconstruction-result reports under `/data2/ego_annotation_outputs/v17_geometry_reconstruction_results`. This layer consumes the prepared RGBD jobs and a BundleSDF output root, then checks whether each solver output has a mesh, a complete per-frame object-in-camera pose sequence, a mesh scale compatible with the rectified RGBD observations, plausible mesh topology, and projection/depth agreement against the job masks and depth maps. Missing backend output is recorded as `pending_solver_output`; a bad mesh is recorded as rejected evidence. The current local report has two detected BundleSDF outputs and zero accepted reconstructions. Both backend runs produced complete pose sequences and `mesh_cleaned.obj`; source inspection shows `mesh_cleaned.obj` is exported in normalized NeRF coordinates before BundleSDF's `mesh_to_real_world` step. The texture path is nonessential for geometry QC and can hang or crash after the mesh exists, so the remote runner now preserves stable `mesh_cleaned.obj` evidence and terminates that path explicitly. V17 recovers a metric QC mesh through BundleSDF's persisted `sc_factor` and translation, records that coordinate contract, and reruns the same scale, topology, projection, and rasterized front-surface depth tests. The recovered metric meshes pass scale and topology, with extents about 34.1 by 38.8 by 27.7 cm for frames 0822-0830 and 29.5 by 36.9 by 26.2 cm for frames 0831-0844. They still fail projection/depth QC: median mask IoU is 0.883 and 0.916, median rasterized front-surface depth error is only 3.4 mm and 1.9 mm, but p95 front-surface depth error reaches about 76 mm and 74 mm. The failure is therefore a tail-depth and visible-surface consistency problem, not a scale-convention failure and not a uniform depth offset. A separate all-frame BundleSDF experiment under `/data2/ego_annotation_outputs/v17_bundlesdf_outputs_allframes` confirmed final NeRF used all 9 and 14 object-pose frames, but it increased p95 depth tails to about 111 mm and 113 mm. A depth-weighted all-frame experiment under `/data2/ego_annotation_outputs/v17_bundlesdf_outputs_allframes_depth100` also failed projection/depth QC with p95 depth tails about 110 mm and 114 mm. These experiments falsify final-NeRF keyframe sparsity and a simple `depth_weight=100` toggle as sufficient fixes. The next object-geometry problem is localizing and correcting the high-tail RGBD/SDF surface disagreement under the prepared masks, depths, and object poses.
