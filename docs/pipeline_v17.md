...
The contact-mode graph is implemented as a V17 contact-state layer, not as the full V3 solver. Its default manifests read the anchor-only sparse graph outputs, so it estimates contact modes from fixed graph-corrected geometry rather than solving hand geometry, object geometry, object pose, and contact labels together. It accepts both representative clips with zero anchor contradictions and explicit `v3_solver_complete=false` metadata. Trash has 1,908 hand-side rows, 1,625 active geometry observations, 281 unobserved rows, 172 contact-mode rows, and 135 contact-factor-ready rows. Tomato has 1,859 hand-side rows, 1,340 active observations, 519 unobserved rows, 527 contact-mode rows, and 386 contact-factor-ready rows. Visual review sheets sampled from the full graph-corrected side-by-side videos show that the long tomato left-hand interval from frame 0714 to 0939 is hand-held washing/rinsing behavior, while the trash contact bursts correspond to bag/trash-can manipulation. Rows without hand/object geometry are marked `unobserved` and cannot inherit temporal contact.

The contact-mode-factor sparse graph consumes only accepted contact-mode `contact_factor_ready` rows through an explicit `--contact-mode-graph-root` input. It accepts both representative clips while preserving the same sparse variable set and incomplete-solver metadata. Trash uses 82 usable contact-mode factors after the hand-quality filter, with 6,560 linearized contact correspondences, contact p95-of-p95 12.0 mm, max object translation correction 28.4 mm, and max hand ray correction 8.1 mm. Tomato uses 381 usable contact-mode factors, with 30,480 linearized contact correspondences, contact p95-of-p95 12.7 mm, max object translation correction 7.8 mm, and max hand ray correction 8.8 mm. Skipped contact-mode rows remain explicit report entries when the current hand geometry fails the sparse solver's hand-quality filter.
...
This section defines the still-open full nonlinear solver target. The implemented V17 graphs are the contact-mode graph and sparse evidence-consistency graphs described in the Status section: the contact-mode graph estimates binary contact/no-contact/unobserved modes from fixed V17 geometry and image evidence, and the sparse geometry graph optimizes object translation and hand camera-ray depth corrections. Camera trajectory, MANO articulation and shape, object mesh topology, object geometry, and nonlinear contact physics stay fixed.
