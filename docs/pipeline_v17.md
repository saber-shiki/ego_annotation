...
The trash anchor repair path now materializes selected VLM-box HaMeR hypotheses as V17 repair hand states for frames 0182, 0260, 0764, 0856, 0949, and 0970. The measurement store also ingests contact evidence recomputed from those repaired states. This clears the hand-state blockers at 0182, 0260, 0764, 0949, and 0970. Frame 0182 has a graph-hand-consistent object-depth candidate for `object:black_trash_bag`, but temporal validation rejects the depth re-anchor because the raw-depth hypothesis already projects onto neighboring masks and the contact-depth hypothesis worsens raw-depth residuals. Frame 0856 has a graph-hand-consistent object-depth repair candidate for `object:white_trash_bag`: the selected right-hand HaMeR state projects onto the SAM2 bag mask, and re-anchoring relative UniDepth to the repaired hand depth makes image-and-metric contact. Temporal validation keeps the candidate unresolved, because only two neighboring frames have comparable white-bag support and camera motion; the object-depth shift cannot be used as accepted contact evidence.
