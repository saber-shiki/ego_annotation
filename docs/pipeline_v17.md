...
The contact-mode-factor sparse graph consumes only accepted contact-mode `contact_factor_ready` rows through an explicit `--contact-mode-graph-root` input. It structurally accepts both representative clips with converged local contact-patch correspondences, but it is not annotation-ready and does not meet the 5 mm target. Trash uses all 81 corrected contact-mode factors, with 1,620 linearized local-patch correspondences, contact-patch p95-of-p95 5.16 mm, broader 80-nearest-point p95-of-p95 12.82 mm, max object translation correction 5.94 mm, max object rotation correction 0.0375 rad, and max hand ray correction 4.17 mm. Tomato uses all 381 corrected contact-mode factors, with 7,620 local-patch correspondences, contact-patch p95-of-p95 5.31 mm, broader 80-nearest-point p95-of-p95 13.09 mm, max object translation correction 8.11 mm, max object rotation correction 0.0407 rad, and max hand ray correction 12.96 mm. The generated sparse reports set `structural_consistency_pass=true`, `accuracy_target_met=false`, `annotation_ready=false`, and `deliverable_ready=false`.
