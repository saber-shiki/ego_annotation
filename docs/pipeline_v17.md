...
The trash anchor repair path now materializes selected VLM-box HaMeR hypotheses as V17 repair hand states for frames 0182, 0260, 0764, 0856, 0949, and 0970. The measurement store also ingests contact evidence recomputed from those repaired states. This clears the hand-state blockers at 0182, 0260, 0764, 0949, and 0970. Frame 0856 now has a validation-pending object-depth repair candidate for `object:white_trash_bag`: SAM2 mask contact covers all repaired-hand vertices, relative UniDepth is re-anchored to repaired hand contact depth, and recomputed contact becomes image-and-metric. The candidate remains unresolved until temporal object-depth validation accepts or rejects that contact-derived depth anchor.
