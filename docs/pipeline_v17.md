...
The contact-mode-factor sparse graph consumes only accepted contact-mode `contact_factor_ready` rows through an explicit `--contact-mode-graph-root` input. It structurally accepts both representative clips while preserving the same sparse variable set and incomplete-solver metadata, but it is not annotation-ready and does not meet the 5 mm target. Trash uses all 81 corrected contact-mode factors, with 6,480 linearized contact correspondences, contact p95-of-p95 12.0 mm, max object translation correction 28.4 mm, and max hand ray correction 8.1 mm. Tomato uses all 381 corrected contact-mode factors, with 30,480 linearized contact correspondences, contact p95-of-p95 12.7 mm, max object translation correction 7.8 mm, and max hand ray correction 8.8 mm. The generated sparse reports set `structural_consistency_pass=true`, `accuracy_target_met=false`, `annotation_ready=false`, and `deliverable_ready=false`.
