...
A valid runtime launch must use an isolated cwd, preferably a dedicated clean git worktree, not the parent agent's active working directory. The runtime agent must run in a dedicated tmux session/window whose purpose is visible to the user, and the launch command must pass only the fixed runtime entrypoint arguments. If the checked-in runtime prompt/system prompt mentions Workbench, `.memory/tasks`, OPS/EPISTEMIC, ablation, GT, parent sessions, or development checklists as runtime instructions, stop and repair the prompt artifacts before launching. Workbench item 2 is not closed until a runtime agent launched under this separation follows the runbook end-to-end on a fresh input/run root.
