...
User-corrected active Workbench as of the renderer-failure diagnosis. Follow these items in order. Prior HOT3D item-6/item-7 outputs are retained as diagnostic evidence only; they are not accepted Workbench completion because the rendered rigid keyboard object was visibly wrong. Do not resume quantitative comparison or autoresearch until the rendered artifact passes subjective physical sanity.

1. Wire the correct renderer into V19. V19 must render a real rigid-body object from render-consumed state, not a diagnostic point cloud from measurement reports. The renderer boundary is state/annotation driven: corrected rigid object geometry and pose trajectory must be explicit state consumed by overlay/world/side-by-side renders.
2. Fix obviously wrong projection. Frame-size/intrinsics/source-size semantics must be consistent. A render projected onto 960x960 frames must not use unscaled 1408x1408 source intrinsics. The check is the visible artifact: projected hand and keyboard layers must align with the raw image in 2D.
3. Rerun the full V19 pipeline using the runtime Pi agent, not parent-assembled stages. The runtime agent must execute end-to-end from input video to required artifacts. The keyboard example must pass subjective sanity: hand and keyboard detections are reasonable in 2D projection; hand and keyboard have the correct spatial relationship in 3D/world view; rigid keyboard body is visible as a body, not a point-cloud substitute; no obvious first-glance faults remain. These are not programmatic gates or escape hatches. The agent must exercise subjective judgment and inspect the artifact as a user would. Each subjective review must include at least one open-ended observation or anomaly that is not merely an answer to the listed review dimensions; the reviewer must look for something categorically new in the artifact, because otherwise the protocol degenerates into another checklist.
4. Improve render quality according to existing instructions. After the corrected artifact is sane, make overlay/world/side-by-side videos audience-readable, full-duration, and honest about unresolved uncertainty.
5. Run quantitative comparison only after subjective sanity passes. HOT3D/open-dataset metrics are useful only after the rendered prediction artifact is physically meaningful; otherwise scoring is a waste and can hide broken mechanisms.
6. Autoresearch and iterate. Controlled research/optimization loops may begin only after the corrected runtime artifact and bounded comparison exist; preserve rendered samples, metrics, failure clusters, and causal interpretations.

Underlying logic: construction precedes measurement. The central deliverable is a rendered physical annotation artifact. Segmentation, residuals, row counts, validators, and metric reports are supporting evidence only when the render-consumed state visibly represents the claimed physical variables. Subjective visual/geometric judgment is mandatory because many failures are obvious to a human and invisible to proxy checks. Subjective judgment is not a fixed checklist: every review must surface at least one unexpected visual fact, anomaly, or risk hypothesis beyond the predeclared review prompts, and that new observation must either change the next repair target or justify why the current artifact has no obvious unlisted fault.
...
A valid runtime launch must use a curated runtime workspace, not the parent agent's active working directory and not a full project worktree. The runtime workspace may contain only the runtime system prompt, runtime prompt template, one authoritative runtime spec, and scripts/config/assets required by that spec. It must not contain `.memory/`, `AGENTS.md`, parent/development docs, evaluator/benchmark docs, historical task reports, or unrelated project documentation. The runtime agent must run in a dedicated tmux session/window whose purpose is visible to the user, and the launch command must pass only the fixed runtime entrypoint arguments. If any runtime-visible prompt/spec mentions Workbench, `.memory/tasks`, OPS/EPISTEMIC, ablation, GT, parent sessions, evaluator roles, or development checklists as runtime instructions, stop and repair the runtime bundle before launching. Workbench item 2 is not closed until a runtime agent launched under this curated-workspace separation follows the spec end-to-end on a fresh input/run root.
