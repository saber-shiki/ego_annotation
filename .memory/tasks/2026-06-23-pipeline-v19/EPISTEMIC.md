...
2. Broader HOT3D slice: Workbench item 6 requires the fixed 3-clip HOT3D slice. `clip-001850` has full runtime object/MANO artifacts and integrated support-gated scoring. `clip-001849` is the active next fixed-slice run: P00-P17 completed under the same runtime pathway after two contract repairs, and P18 support-gated interval MANO is currently running. `clip-001851` has baseline/pinhole evaluation inputs but not a V19 support-gated runtime freeze.
3. `clip-001849` contract repairs: P11 produced a valid object-isolated keyboard RGBA crop, but the first P12 handoff selected the raw full frame, which would have conditioned TRELLIS on the scene rather than the object. Crop-conditioned P12 then succeeded, P13 completed a mesh, and P14/P15 fit 120 visible poses with median residual about `0.00417 m`. P17 also exposed that the factor report is case-nested under the output root; the spec now binds P18 to that concrete path. These were systematic input/output contract bugs, not normal measurement noise (OPS 2026-06-27T06:41:09+08:00 and 2026-06-27T06:52:00+08:00).
4. Contact/nonpenetration: still uncertain. P16 for `clip-001849` reports non-watertight completed/sign meshes, so it cannot prove signed nonpenetration. P17 rows are uncertainty-preserving factor inputs over frames 30-149, not accepted contact labels. The gate prevents unsupported hand translation; it does not prove contact ownership, signed nonpenetration, or object-hand force consistency.
...
Let the runtime-owned `clip001849_p12_cropfix` continuation finish P18-P21. If P18 succeeds, consume rendered P19/P20/P21 artifacts physically, ensure superseded P12 blocker evidence is not treated as active, freeze the prediction boundary, and only then run evidence-only HOT3D MANO scoring. If P18 fails, preserve the precise blocker and repair the interval-solver/input mechanism without evaluator-side scoring or visible-surface fallback.
