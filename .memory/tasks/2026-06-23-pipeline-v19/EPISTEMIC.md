...
Consume the active clip001849 third-slice run: full render, publication/freeze, HOT3D evaluation, matched-row comparison, and physical visual review against the pre-run predictions in OPS. If clip001849 again preserves metrics but confirms the v4 branch is slow or visually noisy, run the prepared direct object-surface posterior as the next Workbench-6 mechanism test. Any next run must preserve the same causal invariant: metric MANO joints remain separate from uncertain contact-surface hypotheses until a mechanism proves joint/root improvement without visual regression.
