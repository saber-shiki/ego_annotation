...
2. Broader HOT3D slice: Workbench item 6 requires the fixed 3-clip HOT3D slice. `clip-001850` has full runtime object/MANO artifacts and integrated support-gated scoring. `clip-001849` is the active next fixed-slice run: P00-P11 completed under the same runtime pathway, P07/P09 evidence supports keyboard identity/localized visible geometry, and a P12 retry is running after a crop-handoff repair. `clip-001851` has baseline/pinhole evaluation inputs but not a V19 support-gated runtime freeze.
3. `clip-001849` P12 crop handoff: P11 produced a valid object-isolated keyboard RGBA crop, but the first P12 handoff selected the raw full frame, which would condition TRELLIS on the scene rather than the object. This is a systematic input-contract bug, not TRELLIS evidence. The live prediction is that crop-conditioned P12 should produce a keyboard-like mesh prior if the handoff was the only blocker; if it fails, the blocker shifts to TRELLIS/model-runtime or crop suitability (OPS 2026-06-27T06:41:09+08:00).
4. Contact/nonpenetration: still uncertain. The gate prevents unsupported hand translation; it does not prove contact ownership, signed nonpenetration, or object-hand force consistency.
...
Let the runtime-owned `clip001849_p12_cropfix` continuation determine whether crop-conditioned P12 can complete. If it succeeds, consume P13-P21 rendered artifacts physically, freeze the prediction boundary, and only then run evidence-only HOT3D MANO scoring. If it fails, preserve the precise P12 blocker and repair the actual TRELLIS/crop-suitability mechanism rather than falling back to visible surfaces, raw frames, labels, or evaluator-side scoring.
