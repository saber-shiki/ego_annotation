...
Run the prepared direct object-surface posterior as the next Workbench-6 mechanism test only after syncing the repaired scripts into the remote bundle. The causal question is whether the useful surface evidence can be rendered as source-MANO-conditioned object-surface support, with lower runtime and honest source-gap uncertainty, without solving a global hand Sim(3) whose metric joints are later discarded. Any next run must preserve the invariant that metric MANO joints remain separate from uncertain contact-surface hypotheses until a mechanism proves joint/root improvement without visual regression.
