...
The first repaired A800 rerun (`v4_coordrigid`) is invalid/abandoned: it reached P03b but the runtime spec left focal extraction underspecified, so P04 began HaWoR with a diagnostic outlier focal (`780.884`) instead of the canonical calibration focal (`559.164`). The spec now explicitly extracts only canonical focal fields and forbids diagnostics scanning. Launch a fresh runtime-owned run from the patched bundle, then require evidence from P07 regenerated keyboard masks, P15 full-timeline rigid pose rows, and rendered frame 0032/0074 before any new freeze/evaluation claim.
