...
- Production-style contact/body rendering originally existed under `/data2/ego_annotation_outputs/research_clip001850_contact_state_20260706/v19_contact_state_full_duration/` and `/v19_contact_state_generic_render/`, but those renders consumed the P15 all-numeric pose graph and drew a localized keyboard body on unknown frames. The render-surface purity audit first detected both root-advertised render blocks as violations; both P15 render dirs and the static-gauge render dir are now under `/data2/ego_annotation_outputs/research_clip001850_contact_state_20260706/superseded_prior_laundering/render_dirs/` and marked superseded negative evidence. Motion coupling is negative on clip001850: no hand-driven object impulse clears the direct-edge motion floor or time-locks to approach, so `confirmed_contact` remains GT-free unreachable. The pose graph is an undriven correction-delta pass-through, not a broken smoother. KT-L1/KT-L2 resolve the object-pose observation route: f30-36,f46 are the only admissible measured keyboard-pose frames; f45 is unresolved; f60/f75/f76/f77 are rejected mask-drift observations; f78-149 are unrecoverable mask delamination; 137 frames have unknown object pose from this measurement path. The rest-cluster reference `T_rest=[0.581,-0.064,0.345] m` describes the observed cluster and rejects outliers only; using it as a missing-frame pose, graph prior, imputed trajectory, contact evidence, or render-localized body is prior laundering. The current pose artifact under `/data2/ego_annotation_outputs/research_clip001850_contact_state_20260706/unknown_preserving_pose_render/` writes numeric R/t only on f30-36,f46, null pose on the other 142 frames, and is now the only current package render surface: keyboard body is drawn only on measured frames. The pose provenance-purity audit under `pose_provenance_purity_audit/` is wired into graph health: current/original/static-fill purity = 1.0/0.0533/0.0533; original nearest-hold/interp rows and static-fill rows are detected as laundering; current package render-surface violations = 0. The reusable pose-state schema contract under `pose_state_schema_contract/` passes 7/7 and enforces observation/unknown/solver-estimate separation beyond clip001850. Remaining R0 work is promotion of this contract into the general sidecar build path; next physical research should move to new evidence (R2/R3 correspondences/geometry epochs or R4 solver-estimate design), not more static-fill cleanup.
