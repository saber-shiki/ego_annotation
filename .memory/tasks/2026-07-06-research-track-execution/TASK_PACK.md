...
- Production-style render consumption exists as a full-duration replacement artifact under `/data2/ego_annotation_outputs/research_clip001850_contact_state_20260706/v19_contact_state_full_duration/`: the durable canonical `contact_frame_detail` drives f32/f36/f45 labels, uncovered frames default to `unresolved_evidence_incomplete`, and the 3221-face observed body is rendered as an open/hatched patch instead of the raw TRELLIS body. A generic parameterized consumer and an `ego.hoi` sidecar now preserve this state, but they are support artifacts. Motion coupling is negative on clip001850: no hand-driven object impulse clears the direct-edge motion floor or time-locks to approach, so `confirmed_contact` remains GT-free unreachable. The pose graph is an undriven correction-delta pass-through, not a broken smoother. KT-L1/KT-L2 resolve the route: clip001850 keyboard is static, f75 is a mask-drift anchor outlier 199.5 mm from rest, f60/f75/f76/f77 are outliers, f78-149 are unrecoverable mask delamination, and direct support cannot rise above 13/150 from existing masks. Next artifact-changing work is a robust static `T_rest`/visibility-ledger render path: re-anchor to the f30-36,f46 rest cluster, reject outliers, attach covariance/observability, mark f45 unresolved, mark f78-149 `mask_unreliable` and no-surface frames `occluded_out_of_view`, and rewrite the pose rows/rendered body placement or uncertainty styling that the renderer consumes.
