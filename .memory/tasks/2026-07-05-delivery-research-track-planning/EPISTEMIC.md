...
Metric mechanism: a useful GT-free metric is disagreement between independent measurement paths. Delivery metrics must route error reduction across the whole target vector: head/camera ATE/RPE/rotation/scale, hand wrist/root, all-joint MPJPE, surface/MPVPE, reprojection, visibility, jitter, caption grounding, and throughput. Fixed-gauge GT/fiducial/IMU evaluation measures head/camera progress toward the 5mm ideal; GT-free camera metrics identify which intervention should reduce the gap. Current HOT3D sub-10 wrist/root result proves smooth correctability under disclosed GT calibration; deployable hand progress now means replacing that GT fit with GT-free self-calibration while preserving joint/surface metrics as active targets.
...
Updated 5mm head/camera model: reaching the ideal is an estimator-and-measurement program. The strongest mechanisms are device VIO/SLAM/IMU/head-pose metadata with calibrated video sync and camera/head extrinsics, controlled fiducial/mocap/metric-anchor capture, and improved visual trajectory estimation tied to metric anchors. RGB-only monocular ego video remains an estimator to improve and score; when its scale/gauge is weak, the next intervention is to add metric constraints and measure the residual gap. The work is to define the metric pose source contract, calibrate synchronization/extrinsics/distortion/rolling shutter, and evaluate ATE/RPE under fixed metric gauge while reducing error on every release.
