...
User correction: the primary delivery artifact is numeric head/camera, hand, caption, and throughput output; renders are QC/demo views and cannot change the delivered numeric result. Prior language that treated rendering as the result was wrong. A render/projection mismatch diagnoses state/K/crop/renderer wiring, but it is not the mechanism for numeric accuracy.

User correction: if HOI is skipped, SAM2/dense object segmentation is not a delivery prerequisite. Throughput analysis must focus on active delivery lanes: pose source ingestion or camera tracking, hand models/fusion, caption/VLM windows, API scheduling, and artifact IO. SAM2 appears only if explicitly chosen for caption grounding or future object requirements.
