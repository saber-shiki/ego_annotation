...
Updated 5mm head/camera model: reaching ~5mm is an input/measurement-source problem, not a claim-policing problem. The feasible paths are device VIO/SLAM/IMU/head-pose metadata with calibrated video sync and camera/head extrinsics, or controlled fiducial/mocap/metric-anchor capture. RGB-only monocular ego video has unobservable metric scale/gauge and cannot by itself support arbitrary ~5mm translation. The work is to define the metric pose source contract, calibrate synchronization/extrinsics/distortion/rolling shutter, and evaluate ATE/RPE under fixed metric gauge.
