...
## 2026-07-06 00:42 — User resolved delivery-track parked decisions
User resolved the remaining delivery choices: customer inputs may include device calibration/VIO/head-pose metadata but the API must only reserve that interface, not depend on it; first hand optimization after wrist/root is all-joint MPJPE; benchmark and deployment choices are the agent's responsibility. Selected benchmark: protected in-house fixed-gauge fiducial/mocap head/camera lockbox with hidden GT, calibrated sync, and known camera/head extrinsics; public sidecar datasets are development/regression checks. Selected simplest deployment: FastAPI job ingress + Ray Serve/Ray GPU actors on a private GPU fleet; Kubernetes/KServe deferred.

## 2026-07-06 00:57 — Rewrote research track around mechanism-driven analysis
User rejected boundary/overclaim framing in the research track and corrected that progress is not blind activity. Rewrote `RESEARCH_TRACK_TASK_PACK_DRAFT.md` so each research line has an artifact defect, physical variable, live mechanisms, discriminating measurement, predictions, intervention, and expected state/render change. Removed boundary/permission framing and replaced it with causal maps, workstream experiments, outcome routing, evaluator construction, RL approximation, distillation, and delivery integration route.
