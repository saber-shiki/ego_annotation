...
- Contact-prior/residual separation is mandatory. A MANO-object or MANO-surfel distance may be rendered as a geometry likelihood/residual only; it cannot define a contact prior and then be used to correct hand/object pose. A contact prior must come from a separately frozen visual-semantic channel (VLM/agent/RGB evidence) and downstream residual artifacts must reference that prior by semantic hash. Raw file checksums are artifact integrity fields, not dependency hashes.
